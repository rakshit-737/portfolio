...
- The night archive: an authored medieval tune — D Dorian on a modelled
  ...
  (never hover, never scroll), and — on fine pointers only — a cursor
  drawn as a medieval key whose padlock appears over anything clickable:
  the click is the unlocking. No audio file
