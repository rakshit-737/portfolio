...
import { benchmarkChart, exhibits } from "@/content";
 ...
 * deliberately not `.ignite`. This table sits in the act's plain `.scrim`
 * reading column (research, not the scheduler act's own `.scrim-wide`),
 * near its right edge (measured directly against the built page: the value
 * column centres at x≈1196px against the lamp's pinned rest x of 666px, a
 ...
 * this width in a plain `.scrim` — no data, semantics, or accessible table
 * structure changed) so it shares the same museum-plate frame every other
 * exhibit uses. Its own former title/unit/source figcaption becomes the
 * `Exhibit` caption (`exhibits.scheduler.caption`, src/content.ts) instead
 * of a duplicate heading inside the frame.
 *
  ...
  return (
    <Exhibit caption={exhibits.scheduler.caption} wide>
      <div ref={ref} data-grown="false">
        ...
        <div
          aria-hidden="true"
          className="relative hidden h-4 border-b border-rule sm:ml-[15rem] sm:block"
        >
          {ticks.map((t) => (
            <span
              key={t}
